...
fn life_cycle() {
    ...
    // 上面就会报错. 因为r引用了x, 而x在[end]位置就已经被释放了,
     ...
     * }
     * 
     *  这里函数的返回值是一个引用类型,且与函数参数没有关系,而是使用了函数体内创建的字符串
     *  这里result在函数结束后就会被释放,但是在函数结束后对result的引用依然继续,
     *  这种情况下没办法指定合适的生命周期来让编译通过
     * 
     *  做好的办法就是返回内部字符串的所有权,然后把字符串的所有权转移给调用者
     * 
